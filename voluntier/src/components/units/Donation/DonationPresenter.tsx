  ...
  const Springprops: ISpringprops = useSpring({
    to: async (next, cancel) => {
      ...
      await next({
        to: {
          val: props.allAmount
            ? Math.floor(Number(props.allAmount?.fetchAllUsersDonationsAmount))
            : 0,
        },
      });
    ...
    to: async (next, cancel) => {
      ...
      await next({
        to: {
          val: props.allAmount
            ? Math.floor(
                Number(props.allAmount?.fetchAllUsersDonationsAmount) / 700
              )
            : 0,
        },
      });
          ...
          <S.TopContents>
            <S.Contents>
              <S.ContentPink>
                <S.Animated>
                  {isTrue
                    ? Springprops.val?.to((val) => Math.floor(Number(val)))
                    : 0}
                </S.Animated>
            ...
            <S.Contents>
              <S.ContentBlue>
                <S.Animated>
                  {isTrue
                    ? Springprops2.val?.to((val) => Math.floor(Number(val)))
                    : 0}
                </S.Animated>
          ...
          <S.MySlider {...settings}>
            <S.SliderContent>
              <S.SliderText>
                여러분의 젤리후원으로
                <br />
                Voluntier는 전국의 유기동물보호센터에
                <br />
                다양한 후원을 하고있습니다!
              </S.SliderText>
            </S.SliderContent>
            <S.SliderContent1>
              <S.SliderText style={{ margin: "8% 0% 0% 45%" }}>
                도움이 필요한 작은 보호센터들의
                <br />
                소중한 마음들이 소외되지 않도록
                <br />
            ...
            <S.SliderContent2>
              <S.SliderText style={{ margin: "23% 0% 0% 5%" }}>
                보내주신 마음 하나하나 헛되이 쓰이지 않도록
                <br />
                Voluntier단이 찾아갑니다
                <br />
              </S.SliderText>
            ...
            <S.SliderContent3>
              <S.SliderText style={{ margin: "10% 0% 0% 5%" }}>
                매달 정기메일을 통해 <br />
                Voluntier의 활동내역을 보실수 있습니다
                <br />
              </S.SliderText>
            </S.SliderContent3>
            <S.SliderContent4>
              <S.SliderText style={{ margin: "20% 0% 0% 10%" }}>
                모든 생명이 존중받고 사랑받는 세상 <br />
                Voluntier가 만들어 가겠습니다
                <br />
              </S.SliderText>
            </S.SliderContent4>
          </S.MySlider>
