...
import * as S from './volunteerList.styles'
  ...
  const onClickUpdateEnroll = (event)=>{
    ...
    try{
      const result = updateenroll({
        ...
        refetchQueries:[{query:FETCH_ENROLL, variables:{boardI: String(router.query.boardId)}}]
  ...
  const onClickDeleteEnroll = (event)=>{
    try{
      ...
      deleteenroll({
        ...
        refetchQueries:[{query:FETCH_ENROLL, variables:{boardId: String(router.query.boardId)}}]
      })
      ...
      <S.ListWrapper>
        <InfiniteScroll>
          {props.enrolldata?.fetchEnrollsByBoardId.map((el: any)=>(
            ...
            <S.Column style={{ width: "50%" }}>{el.user.phone.slice(7,12)}</S.Column>
            <S.Column style={{ width: "30%", color: "#FF3800" }}>
            {el.status === "COMPLETE" ? "봉사완료" : "신청중"}</S.Column>
          {props.boarddata?.user.id === props.Userdata?.fetchLoginUser.id
              ?
                ...
                <S.DeleteIcon id={el.id} onClick={onClickDeleteEnroll} src="/images/boardWrite/delete.png" />
                </S.Column>
                <S.Column style={{ width: "30%" }}>
                <S.CompleteButton id={el.id}
                onClick={onClickUpdateEnroll}
                >완료처리하기</S.CompleteButton>
                </S.Column>
                </>
                }
                </>
